...
public class UserController {
    ...
    String AITalk(@RequestPart(value = "newTalk",required = true) String newTalk,
        ...
        String openId;
        if(Boolean.TRUE.equals(redisTemplate0.hasKey(loginCode))){
            openId=(String)redisTemplate0.opsForValue().get(loginCode);
            if(openId==null){
                return "请求失败，登录码不合法";
            }
        }
        else{
            openId=wechatLoginUtil.checkOpenId(loginCode);
            if(openId==null){
                return "请求失败，登录码不合法";
            }
            redisTemplate0.opsForValue().set(loginCode, openId);
        }
