...
def cv2ImgAddText(img, label, left, top, textColor=(255, 255, 255)):
    ...
    #   设置字体
    #---------------#
    font = ImageFont.truetype(font='./fr_main/model_data/simhei.ttf', size=20)
...
class Retinaface(object):
    _defaults = {
        ...
        #   retinaface训练完的权值路径
        #----------------------------------------------------------------------#
        "retinaface_model_path" : 'fr_main/model_data/Retinaface_resnet50.pth',
        #----------------------------------------------------------------------#
        ...
        #   facenet训练完的权值路径
        #----------------------------------------------------------------------#
        "facenet_model_path"    : 'fr_main/model_data/facenet_mobilenet.pth',
        #----------------------------------------------------------------------#
    ...
    def __init__(self, encoding=0, **kwargs):
        ...
        try:
            self.known_face_encodings = np.load("./fr_main/model_data/face_encoding.npy")
            self.known_face_names     = np.load("./fr_main/model_data/face_names.npy")
        except:
            if not encoding:
    ...
    def encode_face_dataset(self, user_id, image_paths, names):
        face_encodings = []
        for index, path in enumerate(tqdm(image_paths)):
            ...
            with torch.no_grad():
                ...
                if len(boxes_conf_landms) <= 0:
                    raise ValueError(str(names[index])+"未检测到人脸")
        ...
        np.save("fr_main/face_dataset/"+user_id+"/npy/"+"face_encoding.npy",face_encodings)
        np.save("fr_main/face_dataset/"+user_id+"/npy/"+"face_names.npy",names)
